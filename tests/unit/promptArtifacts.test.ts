...
import { createEmptyPromptPayload, type TemplatePayload } from '@/models/promptSchema';
import {
...
describe('syncTemplateWithLinkedMenus', () => {
  test('materializa snapshots dos menus vinculados e remove ids desconhecidos', () => {
    ...
    expect(synced.menu_definitions[0]).toEqual(contextMenuToDefinition(buildContextMenu()));
  });
});
...
describe('syncTemplateWithMenuDefinitions', () => {
  test('hidrata snapshots a partir de definições importadas quando o template só traz menu_ids', () => {
    ...
    const synced = syncTemplateWithMenuDefinitions(template, [importedDefinition]);
            ...
            selections: [
              {
                option_value: 'formal',
                option_label: 'Formal',
                selected_sub_options: [{ value: 'corporativo', label: 'Corporativo' }],
              },
            ],
          },
        },

...
db.version(3).stores({
  ...
  await prompts.toCollection().modify((prompt: Record<string, unknown>) => {
    if (!prompt.enabledMenuIds) {
      const contextMenus = (prompt.contextMenus || {}) as Record<
        string,
        unknown
      >;
...
db.version(5).stores({
  ...
  prompts:
    "++id, categoryId, title, createdAt, updatedAt, remoteId, syncStatus",
  menuOptions: "++id, menuKey, value",
...
db.version(6).stores({
  ...
  menuOptions: "++id, menuKey, value",
  contextMenus:
    "++id, menuId, menuName, selectionMode, createdAt, remoteId, syncStatus",
}).upgrade(async (tx) => {
  const prompts = tx.table("prompts");
  const contextMenus = tx.table("contextMenus");
  ...
  await prompts.toCollection().modify((prompt: Record<string, unknown>) => {
    const promptPayload = createPromptPayloadFromLegacyRecord({
      ...
      systemRole: typeof prompt.systemRole === "string"
        ? prompt.systemRole
        : "",
      ...
      contextMenus:
        prompt.contextMenus && typeof prompt.contextMenus === "object"
          ? (prompt.contextMenus as Record<
            string,
            { option?: string; subOptions?: string[] }
          >)
          : {},
      enabledMenuIds: Array.isArray(prompt.enabledMenuIds)
        ? (prompt.enabledMenuIds as string[])
        : undefined,
      constraints: Array.isArray(prompt.constraints)
        ? (prompt.constraints as string[])
        : [],
      ...
      outputSchema:
        prompt.outputSchema && typeof prompt.outputSchema === "object"
          ...
          : undefined,
      referenceUrl: typeof prompt.referenceUrl === "string"
        ? prompt.referenceUrl
        : undefined,
      language: typeof prompt.language === "string"
        ? prompt.language
        : undefined,
      schemaVersion: typeof prompt.schemaVersion === "string"
...
db.version(8).stores({
  categories: "++id, input, name, createdAt, remoteId, syncStatus",
  prompts:
    "++id, categoryId, title, schemaVersion, language, outputFormat, selectedMenuIds, createdAt, updatedAt, remoteId, syncStatus",
  menuOptions: "++id, menuKey, value",
  contextMenus:
    "++id, menuId, menuName, selectionMode, createdAt, remoteId, syncStatus",
}).upgrade(async (tx) => {
...
db.version(12).stores({
  ...
  await promptMemory.toCollection().modify(
    ...
    try {
      ...
      await db.transaction("rw", db.categories, db.contextMenus, async () => {
        ...
        if (missingCategories.length > 0) {
          console.log(
            `🌱 Seed: Criando ${missingCategories.length} categorias padrão...`,
          );
        ...
        if (missingContextMenus.length > 0) {
          console.log(
            `🌱 Seed: Criando ${missingContextMenus.length} menus hierárquicos padrão...`,
          );
        ...
        for (const defaultPrompt of DEFAULT_PROMPTS) {
          const category = categoriesAfterSeed.find(
            (c) =>
              normalizeKey(c.name) === normalizeKey(defaultPrompt.category),
          );

          if (category?.id) {
            const exists = existingPrompts.some(
              (p) =>
                normalizeKey(p.title) === normalizeKey(defaultPrompt.title),
            );

            if (!exists) {
              console.log(
                `🌱 Seed: Criando prompt padrão "${defaultPrompt.title}"...`,
              );
